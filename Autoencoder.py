...
gnn_channels = 32
es_patience = 1000
methods = ['tracemincut_pool']
# 'diff_pool', 'top_k_pool', 'mincut_pool', 'sag_pool'
...
if dataset == 'ring':
    ...
    G = graphs.SwissRoll(dim=2)
elif dataset == 'two_moons':
    G = graphs.TwoMoons(N=500)
...
plt.tight_layout()
plt.savefig(os.path.join(out_dir, dataset + '.png'))
# plt.show()
...
for method in methods:
    ...
    # pooling
    if method == 'mincut_pool':
        X2, A2, I2, M2 = MinCutPool(k=n_nodes // 4, h=gnn_channels)([X1, A_in, I_in])
    if method == 'tracemincut_pool':
        X2, A2, I2, M2 = TraceMinCutPool(k=n_nodes // 4, h=gnn_channels)([X1, A_in, I_in])
    elif method == 'diff_pool':
        X2, A2, I2, M2 = DiffPool(k=n_nodes // 4, channels=gnn_channels)([X1, A_in, I_in])
    elif method == 'top_k_pool':
        X2, A2, I2, M2 = TopKPool(0.25, return_mask=True)([X1, A_in, I_in])
    elif method == 'sag_pool':
        X2, A2, I2, M2 = SAGPool(0.25, return_mask=True)([X1, A_in, I_in])
    else:
        raise ValueError
    # unpooling
    if method in ['mincut_pool', 'diff_pool', 'tracemincut_pool']:
        X3, A3, I3 = upsampling_from_matrix_op([X2, A2, I2, M2])
    else:
        X3, A3, I3 = upsampling_from_mask_op([X2, A2, I2, M2])
    # decoder
    ...
    plt.axvline(0, c='k', alpha=0.2)
    plt.axhline(0, c='k', alpha=0.2)
    plt.savefig(os.path.join(out_dir, dataset + '_' + method + '.png'))
    # plt.show()
